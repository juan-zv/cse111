def main():
    # for i in range(len(make_periodic_table())):
    #     i += 1
    #     periodic_table = make_periodic_table()[i-1][1:3]
    #     print(f"{str(periodic_table)}")

    chemical_formula = input("Enter a Chemical Formula: ")

    formula_grams = float(input("Enter the amount of grams of the formula: "))

    periodic_table = make_periodic_table()

    formula_molecules = parse_formula(chemical_formula, periodic_table)

    molar_mass = compute_molar_mass(formula_molecules, periodic_table)

    moles = formula_grams / molar_mass
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for _ in symbol_quantity_list:
        # Separate the inner list into symbol and quantity.
            symbol = symbol_quantity_list[index_molecules][0]
            quantity = symbol_quantity_list[index_molecules][1]

        # Get the atomic mass for the symbol from the dictionary.
            atomic_mass = periodic_table_dict[symbol][1]

        # Multiply the atomic mass by the quantity.
            molar_mass = atomic_mass * float(quantity)
